...
type SortKey = "name" | "category" | "location" | "status" | "createdAt";
...
function PortfolioListContent() {
  ...
  function fetchProjects() {
    if (!token) return;
    api
      .get<{ projects: Project[] }>("/api/admin/portfolio", token)
  ...
  function handleExport() {
    ...
    const header = ["No", "Name", "Slug", "Category", "Location", "Status", "Joined Date"];
    ...
    a.href = url;
    a.download = "portfolio-projects.csv";
    a.click();
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      await api.delete(`/api/admin/portfolio/${id}`, token);
      setProjects((prev) => prev?.filter((p) => p.id !== id) ?? null);
      await revalidatePublicPaths(["/", "/portfolio", ...(slug ? [`/portfolio/${slug}`] : [])]);
    } catch (err) {
  ...
  return (
    <AdminShell fullWidth>
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold text-ink">All Portfolio Projects</h1>
          {projects && (
      ...
      <div className="mt-6 rounded-lg border border-line bg-paper shadow-sm overflow-hidden">
        ...
        {projects === null && <p className="p-8 text-sm text-slate text-center">Loading…</p>}

        {projects?.length === 0 && (
          <div className="p-12 text-center">
            <FolderOpen className="h-8 w-8 text-slate/30 mx-auto" strokeWidth={1.5} />
            <p className="mt-3 text-sm text-slate">No projects yet.</p>
            <Link href="/admin/portfolio/new" className="inline-block mt-4">
              <Button size="sm">
        ...
        {paged?.length === 0 && projects && projects.length > 0 && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-line">
                {paged.map((project, i) => (
                  <tr key={project.id} className="hover:bg-paper-dim/50 transition-colors">
                      ...
                      <div className="flex items-center gap-3 min-w-[220px]">
                        <div className="relative h-11 w-16 shrink-0 rounded-md bg-paper-dim border border-line overflow-hidden">
                          {project.imageUrl ? (
                            ...
                            <img
                              src={apiImageUrl(project.imageUrl)}
                              alt=""
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate mt-0.5 truncate">/{project.slug}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-slate">{project.location}</td>
                    <td className="px-5 py-4">
